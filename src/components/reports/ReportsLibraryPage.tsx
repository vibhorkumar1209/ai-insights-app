...
export default function ReportsLibraryPage() {
  ...
  useEffect(() => {
    seedMarketIntelReports().then(() => {
      ...
      setEntries(local);
      // Pull in reports generated by hitting the API directly (curl, another
      // service — anything outside this browser's own session) — see
      // lib/apiReports.ts. Wherever the module has a known History field
      // mapping, syncApiOnlyReports() PERSISTS it via saveToHistory as a
      // side effect (so it becomes a real local entry — View/export work
      // exactly like a browser-generated report, and it won't be re-synced
      // next load) and returns only the genuine leftover cases: modules
      // with no known field mapping, shown as lightweight rows whose View
      // opens the raw report JSON instead of a rendered page.
      syncApiOnlyReports(local).then((apiRawOnly) => {
        const merged = loadHistory(); // re-read — now includes any newly-persisted hydrated entries
        setEntries([...merged, ...apiRawOnly].sort(
  ...
  function handleView(entry: HistoryEntry) {
    // API-sourced rows (see lib/apiReports.ts) have no rich per-module data
                    ...
                    >
                      ...
                      <td style={{ padding: '14px 16px', textAlign: 'right' }}>
                        <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', alignItems: 'center' }} onClick={(e) => e.stopPropagation()}>
                          ...
                          >
                            {entry.sourceKind === 'api-raw' ? 'View Raw' : 'View'}
                          </button>

                          {/* Download dropdown — not available for API-sourced rows,
                              which have no rich per-module data to export from (see
                              lib/apiReports.ts) */}
                          {entry.sourceKind !== 'api-raw' && (
