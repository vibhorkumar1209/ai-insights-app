...
import { fetchApiOnlyReportEntries, apiPathForModuleType } from '@/lib/apiReports';
...
export default function ReportsLibraryPage() {
  ...
  useEffect(() => {
    seedMarketIntelReports().then(() => {
      ...
      setEntries(local);
      // Merge in reports generated by hitting the API directly (curl,
      // another service — anything outside this browser's own session) —
      // see lib/apiReports.ts. These render as lightweight rows (no rich
      // per-module data, so View/Export fall back to a raw-JSON link).
      fetchApiOnlyReportEntries(local).then((apiOnly) => {
        if (apiOnly.length === 0) return;
        setEntries((prev) => [...prev, ...apiOnly].sort(
  ...
  function handleView(entry: HistoryEntry) {
    ...
    if (entry.sourceKind === 'api' && entry.apiJobId) {
  ...
  function handleDelete(id: string) {
    ...
    if (entry?.sourceKind !== 'api') deleteHistoryEntry(id);
