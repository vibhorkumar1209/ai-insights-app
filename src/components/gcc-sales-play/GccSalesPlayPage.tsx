...
import { GccSalesPlayResult } from '@ai-insights/types';
...
const EMPTY_FORM: FormState = { targetCompany: '', advisoryFirm: '', targetGeoRegion: '', coreIndustrySegment: '' };
...
export default function GccSalesPlayPage() {
  const [form, setForm] = useState<FormState>(EMPTY_FORM);
  const [job, setJob] = useState<GccSalesPlayResult | null>(null);
  ...
  useEffect(() => {
    ...
    if (pendingId) {
      ...
      if (entry?.moduleType === 'gcc-sales-play' && entry.gccSalesPlayData) {
        restoreEntry(entry);
      }
    }
  ...
  function restoreEntry(entry: HistoryEntry) {
    if (entry.moduleType !== 'gcc-sales-play' || !entry.gccSalesPlayData) return;
    const d = entry.gccSalesPlayData;
    setForm({
      targetCompany: d.targetCompany,
      advisoryFirm: d.advisoryFirm,
      targetGeoRegion: d.targetGeoRegion,
      coreIndustrySegment: d.coreIndustrySegment,
    });
    setJob(d);
    setError(null);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(API_ENDPOINTS.gccSalesPlay, {
        method: 'POST',
      ...
      const es = new EventSource(API_ENDPOINTS.gccSalesPlayStream(jobId));
      esRef.current = es;

      es.addEventListener('progress', (ev) => {
        const data = JSON.parse((ev as MessageEvent).data) as Partial<GccSalesPlayResult>;
        setJob((prev) => ({ ...(prev ?? {} as GccSalesPlayResult), ...data }));
      });
      es.addEventListener('result', (ev) => {
        const data = JSON.parse((ev as MessageEvent).data) as GccSalesPlayResult;
        setJob(data);
        es.close();
        if (data.status === 'complete') {
          saveToHistory({
            moduleType: 'gcc-sales-play',
            targetCompany: form.targetCompany,
            completedAt: new Date().toISOString(),
            gccSalesPlayData: data,
          });
          setHistoryCount(loadHistory().length);
        }
      });
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FFFFFF', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <ModuleIcon id="gcc-sales-play" size={20} />
              <span style={{ fontSize: 18, fontWeight: 800, color: '#FFFFFF' }}>GCC Sales Play</span>
            </div>
          </div>
          <button
        ...
        {!isDone && (
          <div style={{ background: 'linear-gradient(135deg, #0c3649, #12516E)', border: '1px solid #CCDFEA', borderRadius: 12, padding: 32 }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              >
                {isRunning ? 'Generating dossier (this takes 3–5 minutes)…' : 'Generate Dossier →'}
              </button>
            </form>
          </div>
        )}
        ...
        {isDone && job && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
              <div>
                <div style={{ fontSize: 20, fontWeight: 700, color: '#1B2A3D' }}>{job.targetCompany} → {job.advisoryFirm}</div>
                <div style={{ fontSize: 13, color: '#6B7280', marginTop: 4 }}>
                  {job.targetGeoRegion} · {job.coreIndustrySegment}
                </div>
              </div>
              <button
                onClick={handleReset}
            ...
            <div className="gcc-markdown" style={{ background: '#F3F8FA', border: '1px solid #CCDFEA', borderRadius: 14, padding: '32px' }}>
      ...
      <style>{`
        .gcc-markdown h2 { font-size: 20px; font-weight: 700; color: #1B2A3D; margin: 28px 0 12px; padding-bottom: 8px; border-bottom: 2px solid rgba(52,145,232,0.25); }
        .gcc-markdown h2:first-child { margin-top: 0; }
        .gcc-markdown h3 { font-size: 16px; font-weight: 700; color: #3491E8; margin: 22px 0 10px; }
        .gcc-markdown h4 { font-size: 14px; font-weight: 700; color: #1B2A3D; margin: 16px 0 8px; }
        .gcc-markdown p { font-size: 13.5px; line-height: 1.7; color: #374B5C; margin: 0 0 12px; }
        .gcc-markdown ul, .gcc-markdown ol { margin: 0 0 12px; padding-left: 22px; }
        .gcc-markdown li { font-size: 13.5px; line-height: 1.7; color: #374B5C; margin-bottom: 4px; }
        .gcc-markdown strong { color: #1B2A3D; }
        .gcc-markdown table { width: 100%; border-collapse: collapse; margin: 12px 0 20px; font-size: 12px; background: #EDF4F8; border-radius: 10px; overflow: hidden; }
        .gcc-markdown th { text-align: left; padding: 10px 12px; background: rgba(52,145,232,0.08); color: #3491E8; font-weight: 700; font-size: 11px; letter-spacing: 0.3px; border-bottom: 1px solid rgba(30,74,104,0.35); white-space: nowrap; }
        .gcc-markdown td { padding: 9px 12px; color: #374B5C; border-bottom: 1px solid rgba(30,74,104,0.15); vertical-align: top; }
        .gcc-markdown tr:nth-child(even) td { background: #F0F7FB; }
        .gcc-markdown table td:first-child { font-weight: 600; color: #1B2A3D; }
        .gcc-markdown hr { border: none; border-top: 1px solid rgba(30,74,104,0.2); margin: 24px 0; }
        .gcc-markdown blockquote { border-left: 3px solid #3491E8; padding-left: 14px; margin: 12px 0; color: #5A6E7A; font-style: italic; }
        .gcc-markdown code { background: rgba(52,145,232,0.08); padding: 1px 5px; border-radius: 4px; font-size: 12px; }
      `}</style>
    </div>
  );
}
