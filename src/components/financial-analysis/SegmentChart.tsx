...
import { FinancialSegmentRow } from '@/lib/types';

interface SegmentChartProps {
  data: FinancialSegmentRow[];
  title: string;
}
...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div style={{
      ...
      borderRadius: 8, padding: '10px 14px', fontSize: 12, maxWidth: 220,
    }}>
      <div style={{ fontWeight: 700, color: '#E8EDF5', marginBottom: 4 }}>{d.segment}</div>
      <div style={{ color: '#22D3EE' }}>Revenue: {d.revenue}</div>
      <div style={{ color: '#7eaabf' }}>Share: {d.percentage}%</div>
      {d.yoyGrowth && <div style={{ color: d.yoyGrowth.startsWith('-') ? '#E63946' : '#10B981' }}>YoY: {d.yoyGrowth}</div>}
    </div>
  );
};

export default function SegmentChart({ data, title }: SegmentChartProps) {
  ...
  return (
    <div>
      ...
      <div style={{ height: Math.max(180, sorted.length * 38 + 40) }}>
